...
class GenerateRandomUsersView(APIView):
    def post(self, request):
        ...
        if not num_of_users or num_of_users < 0 or num_of_users > 100:
            return Response({"error": "Number of users should be specified and should not exceed 100."}, status=400)

        try:
            response = requests.get(f"https://random-data-api.com/api/v2/users/?size={num_of_users}&response_type=json")
            ...
            if response.status_code == 200:
                ...
                return Response({"message": f"{num_of_users} users have been successfully added."}, status=201)
            else:
...
class AddBankToUserView(APIView):
    def post(self, request):
        ...
        if user.banks.filter(pk=bank_id).exists():
            return Response({'error': 'User is already associated with this bank'}, status=400)
